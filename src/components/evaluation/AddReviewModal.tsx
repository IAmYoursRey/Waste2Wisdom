...
export const AddReviewModal: React.FC<AddReviewModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (rating < 1 || rating > 5) {
      addToast('Harap berikan rating bintang antara 1 sampai 5', 'warning');
      return;
    }
    if (comment.trim().length < 15) {
      addToast('Ulasan evaluasi minimal terdiri dari 15 karakter agar informatif bagi pembaca lain.', 'warning');
      return;
    }
    ...
    addToast('Ulasan evaluasi berhasil disimpan ke sistem!', 'success');
